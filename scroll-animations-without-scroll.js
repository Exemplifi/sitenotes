...
class ScrollAnimations {
    ...
    init() {
        ...
        // Removed scroll-driven animation loop; using stepwise orchestration instead
        ...
        // Stepwise sequence: run animations in order when content-container is in view
        this.setupStepAnimationsOnVisibility();
    ...
    // pause/idle logic removed as requested
    ...
    // Stepwise orchestration when container (desktop) or svg-column (mobile) is in view
    setupStepAnimationsOnVisibility() {
        const desktopTarget = document.querySelector('.content-container');
        const mobileTarget = document.querySelector('.svg-column');
        if (!desktopTarget && !mobileTarget) return;

        this._stepSequenceRunning = false;
        this._stepSequenceCompleted = false;
        this._sequenceToken = 0;
        this._cancelSequence = false;

        const runSection = async (section, token) => {
            if (this._cancelSequence || this._sequenceToken !== token) return;
            // Step 1: 20% phase → show line (base)
            this.triggerLinePhase(section);
            await this._wait(1400); // line transition ~1.3s + buffer
            if (this._cancelSequence || this._sequenceToken !== token) return;

            // Step 2: 50% phase → show skeleton + popup
            this.triggerFiftyPhase(section);
            await this._wait(1300); // popup/skeleton settle (1.2s + margin)
            if (this._cancelSequence || this._sequenceToken !== token) return;

            // Step 3: 80% phase → exit transitions
            this.triggerEightyExit(section);
            await this._wait(1000);
            if (this._cancelSequence || this._sequenceToken !== token) return;

            // Clear icon emphasis after exit, then 1s gap
            this.clearIcons(section);
            await this._wait(1200);
        };

        const runSequence = async () => {
            if (this._stepSequenceRunning || this._stepSequenceCompleted) return;
            this._stepSequenceRunning = true;
            this._cancelSequence = false;
            const token = ++this._sequenceToken;

            // Ensure fresh initial state
            this.resetAllAnimationsToInitial();

            await runSection('one', token);
            if (this._cancelSequence || this._sequenceToken !== token) { this._stepSequenceRunning = false; return; }
            await runSection('two', token);
            if (this._cancelSequence || this._sequenceToken !== token) { this._stepSequenceRunning = false; return; }
            await runSection('three', token);
            if (this._cancelSequence || this._sequenceToken !== token) { this._stepSequenceRunning = false; return; }

            this._stepSequenceRunning = false;
            this._stepSequenceCompleted = true;
        };

        const observer = new IntersectionObserver((entries) => {
            entries.forEach((entry) => {
                const isDesktop = !window.matchMedia('(max-width: 600px)').matches;
                const activeTarget = isDesktop ? desktopTarget : mobileTarget;
                if (!activeTarget || entry.target !== activeTarget) return;

                const ratio = entry.intersectionRatio || 0;
                if (ratio >= 0.4) { // start only when 40% visible
                    runSequence();
                } else if (ratio < 0.4) {
                    // Cancel current run as soon as we dip below start threshold
                    this._cancelSequence = true;
                    this._stepSequenceRunning = false;
                    this._stepSequenceCompleted = false;
                    this.resetAllAnimationsToInitial();
                }
    ...
    triggerSectionBase(section) {
        // Lines
        const lineKey = `lineAnimation${section}`;
        if (this[lineKey] && typeof this[lineKey].trigger === 'function') {
            this[lineKey].trigger();
        }

        // Skeleton
        const skeletonKey = `skeletonAnimation${section}`;
        if (this[skeletonKey] && typeof this[skeletonKey].trigger === 'function') {
            this[skeletonKey].trigger();
            if (section === 'one') this.skeletonBaseTriggeredOne = true;
            if (section === 'two') this.skeletonBaseTriggeredTwo = true;
        }

        // Popups
        if (section === 'one' && this.popupOneAnimation) {
            this.popupOneAnimation.trigger();
        }
        if (section === 'two' && this.popupTwoAnimation) {
            this.popupTwoAnimation.trigger();
        }
        if (section === 'three') {
            if (this.popupThreeAnimation) this.popupThreeAnimation.trigger();
            if (this.popupFourAnimation) setTimeout(() => this.popupFourAnimation.trigger(), 200);
        }

        // Icons emphasis apply during base state
    ...
    // Reset all animations back to their initial, pre-trigger state
    resetAllAnimationsToInitial() {
        // Reset line animations
        ['one', 'two', 'three'].forEach((section) => {
            const lineKey = `lineAnimation${section}`;
            const anim = this[lineKey];
            if (anim && anim.elements) {
                ...
                anim.reverseTriggered = false;
                anim.eightyTriggered = false;
            }
        });

        // Reset skeletons
        ['one', 'two', 'three'].forEach((section) => {
            const sk = this[`skeletonAnimation${section}`];
            if (sk && sk.element) {
                sk.element.style.transition = 'none';
                sk.element.style.opacity = '0';
                sk.triggered = false;
                sk.reverseTriggered = false;
                sk.eightyTriggered = false;
            }
        });

        // Reset popups
        if (this.popupOneAnimation && this.popupOneAnimation.element) {
            const el = this.popupOneAnimation.element;
        ...
        if (this.popupTwoAnimation && this.popupTwoAnimation.element) {
            const el = this.popupTwoAnimation.element;
            el.style.transition = 'none';
            el.style.opacity = '0';
            el.style.transform = 'translate(-10px, 10px)';
        ...
        if (this.popupThreeAnimation && this.popupThreeAnimation.element) {
            const el = this.popupThreeAnimation.element;
            el.style.transition = 'none';
            el.style.opacity = '0';
            el.style.transform = 'translate(-10px, 10px)';
            this.popupThreeAnimation.triggered = false;
        ...
        if (this.popupFourAnimation && this.popupFourAnimation.element) {
            const el = this.popupFourAnimation.element;
            el.style.transition = 'none';
            el.style.opacity = '0';
            el.style.transform = 'translate(-10px, 10px)';
            this.popupFourAnimation.triggered = false;
    ...
    // Helpers to trigger specific phases explicitly
    triggerLinePhase(section) {
        const lineKey = `lineAnimation${section}`;
        if (this[lineKey] && typeof this[lineKey].trigger === 'function') {
            this[lineKey].trigger();
        }
    ...
    triggerFiftyPhase(section) {
        const sk = this[`skeletonAnimation${section}`];
        if (sk && typeof sk.trigger === 'function') sk.trigger();

        if (section === 'one' && this.popupOneAnimation) this.popupOneAnimation.trigger();
        if (section === 'two' && this.popupTwoAnimation) this.popupTwoAnimation.trigger();
        if (section === 'three') {
            if (this.popupThreeAnimation) this.popupThreeAnimation.trigger();
            if (this.popupFourAnimation) setTimeout(() => this.popupFourAnimation.trigger(), 200);
        }
    ...
    triggerEightyExit(section) {
        this.handleLineAnimationEighty(80, section);
        this.handleSkeletonAnimationEighty(80, section);
        if (section === 'one' || section === 'two') {
            this.handlePopupAnimationEighty(80, section);
        } else if (section === 'three') {
            this.handlePopupThreeAnimationEighty(80);
            this.handlePopupFourAnimationEighty(80);
        }
    }
    ...
    setupLineAnimation(section) {
        ...
        if (lineElement) {
            ...
            this[`lineAnimation${section}`].elements.forEach(element => {
                element.style.transition = 'stroke-dashoffset 1.3s ease-out, opacity 1.3s ease-out';
                        element.style.strokeDashoffset = '0';
                        element.style.opacity = '1';
                    });
            ...
            this[`lineAnimation${section}`].elements.forEach(element => {
                const length = element.getTotalLength();
                element.style.transition = 'stroke-dashoffset 1.1s ease, opacity 1.1s ease';
    ...
    setupPopupThreeAnimation() {
        ...
        if (popupElement) {
            ...
            popupElement.style.transition = 'opacity 1.2s ease-out, transform 1.2s ease-out';
    ...
    setupSkeletonScreenAnimation(section) {
        ...
        if (skeletonElement) {
            ...
            skeletonElement.style.transition = 'opacity 0.9s ease';
            ...
            this[`skeletonAnimation${section}`] = {
                ...
                trigger: () => {
                    ...
                    // Instant entry (no transition on fade-in)
                    this[`skeletonAnimation${section}`].element.style.transition = 'none';
                ...
                reverse: () => {
                    ...
                    // Keep existing fade-out speed
                    this[`skeletonAnimation${section}`].element.style.transition = 'opacity 0.9s ease';
        ...
        if (popupElement) {
            ...
            this[animationKey] = {
                ...
                trigger: () => {
                    ...
                    this[animationKey].element.style.transition = 'opacity 1.2s ease-out, transform 1.2s ease-out';
                ...
                reverse: () => {
                    ...
                    // Keep fade-out quicker
                    this[animationKey].element.style.transition = 'opacity 0.9s ease, transform 0.9s ease';
    ...
    handleSkeletonAnimationEighty(scrollPercent, section) {
        ...
        this[animKey].element.style.transition = 'opacity 0.9s ease';
    ...
    handleSkeletonAnimationEightyReverse(scrollPercent, section) {
        ...
        if (scrollPercent >= 50) {
            // Instant show when coming back below 80%
            this[animKey].element.style.transition = 'none';
            this[animKey].element.style.opacity = '1';
        } else {
            this[animKey].element.style.transition = 'opacity 0.9s ease';
            this[animKey].element.style.opacity = '0';
        }
    ...
    handlePopupAnimationEighty(scrollPercent, section) {
        ...
        this[animationKey].element.style.transition = 'opacity 1.1s ease, transform 1.1s ease';
        this[animationKey].element.style.opacity = '0';
        this[animationKey].element.style.transform = 'translate(10px, 10px)';
        this[animationKey].eightyTriggered = true;
    }
    ...
    handlePopupThreeAnimationEighty(scrollPercent) {
        ...
        this.popupThreeAnimation.element.style.transition = 'opacity 1.1s ease, transform 1.1s ease';
        this.popupThreeAnimation.element.style.opacity = '0';
        this.popupThreeAnimation.element.style.transform = 'translate(10px, 10px)';
        this.popupThreeAnimation.eightyTriggered = true;
    }
    ...
    handlePopupThreeAnimationEightyReverse(scrollPercent) {
        ...
        this.popupThreeAnimation.element.style.transition = 'opacity 0.9s ease, transform 0.9s ease';
        this.popupThreeAnimation.element.style.opacity = '1';
        this.popupThreeAnimation.element.style.transform = 'translate(0, 0)';
        this.popupThreeAnimation.eightyTriggered = false;
    }
    ...
    handlePopupFourAnimationEighty(scrollPercent) {
        ...
        this.popupFourAnimation.element.style.transition = 'opacity 1.1s ease, transform 1.1s ease';
        this.popupFourAnimation.element.style.opacity = '0';
        this.popupFourAnimation.element.style.transform = 'translate(10px, 10px)';
        this.popupFourAnimation.eightyTriggered = true;
    }
    ...
    handlePopupFourAnimationEightyReverse(scrollPercent) {
        ...
        this.popupFourAnimation.element.style.transition = 'opacity 0.9s ease, transform 0.9s ease';
        this.popupFourAnimation.element.style.opacity = '1';
        this.popupFourAnimation.element.style.transform = 'translate(0, 0)';
        this.popupFourAnimation.eightyTriggered = false;
    }
