...
class ScrollAnimations {
    constructor() {
        ...
        this.observer = null;
        // Enable step-based sequencing for this variant
        this.stepMode = true;
    ...
    init() {
        ...
        // Step-based mode replaces continuous scroll-driven loop
        if (this.stepMode) {
            this.setupStepScrollController();
        } else {
    ...
    // Step-based controller: trigger full section sequences on scroll gestures
    setupStepScrollController() {
        ...
        this._currentSectionIndex = -1; // none yet
        this._enterDelayTimer = null;

        const sections = ['one', 'two', 'three'];

        const runSection = async (index) => {
            if (this._stepSequenceRunning) return;
            const section = sections[index];
            if (!section) return;
            this._stepSequenceRunning = true;
            // Reset to a clean base before running the next one
            this.resetAllAnimationsToInitial();

            // Full sequence: line → 50% (skeleton+popup) → 80% exit
        ...
        // On enter, after 1s, play section one once
        const observer = new IntersectionObserver((entries) => {
            if (!entries || !entries.length) return;
            const entry = entries[0];
            const ratio = entry.intersectionRatio || 0;
            if (ratio >= 0.4) {
                if (this._currentSectionIndex < 0 && !this._enterDelayTimer) {
                    this._enterDelayTimer = setTimeout(() => {
                        runSection(0);
                        this._enterDelayTimer = null;
                    }, 1000);
                }
            } else {
                if (this._enterDelayTimer) {
                    clearTimeout(this._enterDelayTimer);
                    this._enterDelayTimer = null;
                }
            }
        }, { threshold: [0, 0.4] });
        ...
        // Wheel gestures to advance/regress sections
        const onWheel = (event) => {
            if (this._stepSequenceRunning) return;
            ...
            if (delta > 0) {
                ...
                if (this._currentSectionIndex < 0) {
                    runSection(0);
                } else if (this._currentSectionIndex < sections.length - 1) {
                    runSection(this._currentSectionIndex + 1);
                }
            } else if (delta < 0) {
                // up → previous
                if (this._currentSectionIndex > 0) {
        ...
        activeTarget.addEventListener('wheel', onWheel, { passive: true });
        ...
        activeTarget.addEventListener('touchend', () => { lastTouchY = null; }, { passive: true });
        activeTarget.addEventListener('touchmove', (e) => {
            ...
            if (dy > threshold) {
                // swipe up (content moves down) → next
                if (this._currentSectionIndex < 0) {
                    runSection(0);
                } else if (this._currentSectionIndex < sections.length - 1) {
                    runSection(this._currentSectionIndex + 1);
    ...
    // Helpers to run phases explicitly
    ...
    triggerFiftyPhase(section) {
        ...
        if (section === 'three') {
            ...
            if (this.popupFourAnimation) this.popupFourAnimation.trigger();
        }
    }
